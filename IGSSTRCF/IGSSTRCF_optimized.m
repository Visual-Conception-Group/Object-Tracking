% This function implements the IGSSTRCF tracker.

function [state, location,time] = IGSSTRCF_optimized(state, im, params,time)
...
%% main loop
    if state.frame > 1
%         state.ifcompress=state.ifcompress1;
...
% use detection features
    ...
    if state.frame == 1
        state.model_q = gpuArray(single(ones(1,size(state.model_xf,3))));
%         state.model_q = state.model_q/norm(state.model_q);
        state.model_q = reshape(state.model_q,[1,1,size(state.model_xf,3)]);
    end
...
% ADMM solution    
   ...
   % ADMM solution for localization
    [state.g_f,state.h_f,state.q,state.past_h]=ADMM_solve_h(params,state.use_sz,state.model_xf,state.yf,state.small_filter_sz,state.w,state.model_w,state.frame,state.model_q,state.past_h,state);
    for iteration = 1:params.al_iteration-1
        [state.w]=ADMM_solve_w(params,state.use_sz,state.model_w,state.h_f,state);
        [state.g_f,state.h_f,state.q,state.past_h]=ADMM_solve_h(params,state.use_sz,state.model_xf,state.yf,state.small_filter_sz,state.w,state.model_w,state.frame,state.model_q,state.past_h,state);
    end
    state.model_w=gather(state.model_w);
    state.w=gather(state.w);
    state.model_q=gather(state.model_q);
    state.q=gather(state.q);
    state.q=double(state.q);
    state.model_q=double(state.model_q);
    
    state.model_w=alphaw*state.w+(1-alphaw)*state.model_w;
    state.model_q=alphaq*state.q+(1-alphaq)*state.model_q;
...
%=============qualitative===================
% ttl = [state.vid '#' num2str(state.frame)];
%     figure(1)
% % % %     subplot(1,3,1)
% %     imshow(multires_pixel_template(:,:,:,3))
%     imshow(im)
%     hold on
    rectangle('Position',rect_position_vis)
%     title(ttl)
%      nm=[num2str(state.frame) '.jpg']
% saveas(gca,nm)
% hold on
%=======================qualitative end================
...
state.position = state.pos([2,1]);
    state.size = state.target_sz([2,1]); %avot2017
