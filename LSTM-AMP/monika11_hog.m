...
function [input1,input2,input3,l1,l2,l3]=monika11_hog(experts,zf,pos,target_sz,cell_size,im,frame,image_fl,rects)
...
im8 = imread(image_fl{frame-8});
im9 = imread(image_fl{frame-9});
im10 = imread(image_fl{frame-10});
im11 = imread(image_fl{frame-11});
elseif frame==11
    im = imread(image_fl{frame});
im1 = imread(image_fl{frame-1});
im2 = imread(image_fl{frame-2});
...
if frame>11
f1=imcrop(img11, gt(frame-11,:));
f2=imcrop(img10, gt(frame-10,:));
f3=imcrop(img9, gt(frame-9,:));
f4=imcrop(img8, gt(frame-8,:));
f5=imcrop(img7, gt(frame-7,:));
f6=imcrop(img6, gt(frame-6,:));
f7=imcrop(img5, gt(frame-5,:));
f8=imcrop(img4, gt(frame-4,:));
f9=imcrop(img3, gt(frame-3,:));
f10=imcrop(img2, gt(frame-2,:));
f11=imcrop(img1, gt(frame-1,:));
elseif frame==11
    f1=zeros([32,32]);
f2=imcrop(img10, gt(frame-10,:));
f3=imcrop(img9, gt(frame-9,:));
...
f9=imresize(f9,[32,32]);
f10=imresize(f10,[32,32]);
f11=imresize(f11,[32,32]);
...
f9 = extractHOGFeatures(f9,'CellSize',[4 4]);
f10 = extractHOGFeatures(f10,'CellSize',[4 4]);
f11 = extractHOGFeatures(f11,'CellSize',[4 4]);
...
f9=reshape(f9,[1764,1]);
f10=reshape(f10,[1764,1]);
f11=reshape(f11,[1764,1]);
